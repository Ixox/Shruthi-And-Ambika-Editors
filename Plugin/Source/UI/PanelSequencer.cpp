...
PanelSequencer::PanelSequencer ()
{
    ...
    seqBpm->setComboParams(seqBmpComboItems, 38);
    seqBpm->setSliderParams(40.0f, 240.0f, 0);
    seqBpm->addListener(this);

    //ScopedPointer<ComboBox> seqGoove;
    addAndMakeVisible(seqGroove = new ComboBox("Seq Groove"));
    seqGroove->setEditableText(false);
    seqGroove->setJustificationType(Justification::centred);
    seqGroove->setColour(ComboBox::buttonColourId, Colours::blue);
    for (int i = 0; seqGrooveItems[i] != nullptr; i++) {
        seqGroove->addItem(seqGrooveItems[i], i + 1);
    }
    seqGroove->setSelectedId(1);
    seqGroove->setScrollWheelEnabled(true);
    seqGroove->addListener(this);

    //ScopedPointer<Slider> seqAmount;
    addAndMakeVisible(seqAmount = new SliderPfm2("Seq Amount"));
    seqAmount->setRange(0, 127, 1);
    seqAmount->setSliderStyle(Slider::RotaryVerticalDrag);
    seqAmount->setTextBoxStyle(Slider::TextBoxAbove, false, 30, 16);
    seqAmount->setDoubleClickReturnValue(true, 0.0f);
    seqAmount->addListener(this);
    ...
    for (int s = 0; s < NUMBER_OF_STEPS; s++) {
        ...
        addAndMakeVisible(seqController[s] = new SliderPfm2(namePreffix + "Controller"));
        seqController[s]->setRange(0.0f, 15.0f, 1.00f);
        seqController[s]->setSliderStyle(Slider::LinearVertical);
        seqController[s]->setTextBoxStyle(Slider::TextBoxBelow, true, 35, 16);
        seqController[s]->setTextBoxIsEditable(false);
        seqController[s]->setDoubleClickReturnValue(true, 0.0f);
        seqController[s]->setValue(0.0f, dontSendNotification);
        seqController[s]->addListener(this);
...
void PanelSequencer::resized()
{
    ...
    seqBpm->setBounds(cBounds.reduced(10, 0));
    ...
    seqGroove->setBounds(cBounds.reduced(10, reducedHeightForCombo));

    cBounds = tempoBounds.removeFromLeft(tempoBounds.getWidth() / numberOfItems--);
    seqAllLabels[3]->setBounds(cBounds.removeFromTop(20));
    seqAmount->setBounds(cBounds.reduced(10, 10));
...
//         seqLabels[l]->setBounds(lablelsBounds.removeFromTop(lablelsBounds.getHeight() / (4 - l)));
    ...
    for (int s = 0; s < 16; s++) {
        ...
        seqController[step]->setBounds(stepBounds.removeFromTop(stepBounds.getHeight() ).reduced(0, 3));
    }
...
void PanelSequencer::buttonClicked(Button *buttonThatWasClicked) {
    if (buttonThatWasClicked == seqPullButton) {
        ...
        for (int s = 0; s < NUMBER_OF_STEPS; s++) {
            ...
            seqController[s]->setValue(rand() % 16);
...
shruthi::SequenceStep* PanelSequencer::getSequencerSteps() {
    updateSequencerSteps();
    return sequencerSettings.steps;
}
...
void PanelSequencer::updateSequencerSteps() {
    for (int s = 0; s < 16; s++) {
        ...
        data2 |= ((int)seqController[s]->getValue());
        if (seqRhythmic[s]->getSelectedId() == 2) {
            data2 |= 0x80;
        }
...
void PanelSequencer::setSequencerSteps(shruthi::SequenceStep newSteps[16]) {

    DBG("PanelEngine::setSequencerSteps() !!!!!!!!!!!!!! ");
    int newNotes[16];
    int newEvents[16];
    for (int b = 0; b < 16; b++) {
        sequencerSettings.steps[b] = newSteps[b];
        ...
        seqVelocity[b]->setValue(newSteps[b].velocity() >> 4);
        seqController[b]->setValue(newSteps[b].controller());
        seqNoteOctave[b]->setSelectedId((newSteps[b].note() / 12) - 1);
        if (newSteps[b].gate()) {
            if (newSteps[b].legato()) {
    ...
    if (comboBoxThatHasChanged == seqNumberOfStep) {
        ...
        if (stepMax >= 1 && stepMax <= 16) {
            for (int s = 1; s <= stepMax; s++) {
                ...
                seqController[s - 1]->setEnabled(true);
            }
            if (stepMax < 16) {
                for (int s = stepMax + 1; s <= 16; s++) {
                    ...
                    seqController[s - 1]->setEnabled(false);
                }
            }
